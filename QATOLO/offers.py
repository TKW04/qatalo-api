...
def _resp(s,b): return {"statusCode":s,"headers":CORS,"body":json.dumps(b,default=str)}
def _now(): return datetime.now().strftime("%Y-%m-%d %H:%M:%S")
def _today(): return datetime.now().strftime("%Y-%m-%d")

def _get_biz(user_id):
    items = business_table.scan(FilterExpression=Attr("user_id").eq(user_id)).get("Items",[])
...
def offers_routes(path, method, event, user_id, alias):
    ...
    # Admin
    if path == f"/{alias}/offers" and method == "GET":  return get_offers(user_id)
    if path == f"/{alias}/offers" and method == "POST": return create_offer(event, user_id)
    m = re.fullmatch(rf"/{alias}/offers/([^/]+)", path)
    if m:
        oid = m.group(1)
        if method == "PUT":    return update_offer(event, oid, user_id)
        if method == "DELETE": return delete_offer(oid, user_id)
...
def get_offers(user_id):
    try:
        ...
        if not biz: return _resp(200, [])
        items = offers_table.scan(FilterExpression=Attr("business_id").eq(biz["business_id"])).get("Items",[])
        return _resp(200, sorted([_map(i) for i in items], key=lambda x: x["create_date"], reverse=True))
...
def _build_item(data, biz_id, offer_id=None):
    ...
    mu   = int(data["max_uses"]) if data.get("max_uses") not in (None,"",0,"0") else None
    item = {
...
def create_offer(event, user_id):
    try:
        biz = _get_biz(user_id)
        if not biz: return _resp(404, {"message":"Negocio no encontrado"})
        data = json.loads(event.get("body","{}"))
        item = _build_item(data, biz["business_id"])
        item["uses_count"] = 0
        item["create_date"] = _now()
        offers_table.put_item(Item=item)
        return _resp(200, {"message":"Oferta creada correctamente","offer_id":item["offer_id"]})
    except Exception as e:
...
def update_offer(event, offer_id, user_id):
    try:
        ...
        if not existing: return _resp(404, {"message":"Oferta no encontrada"})
        biz = _get_biz(user_id)
        ...
        offers_table.update_item(
            ...
            ExpressionAttributeValues={
                ":n":item["name"],":d":item["description"],":a":item["is_active"],
                ":tr":item["trigger"],":c":code,":dt":item["discount_type"],
                ":dv":item["discount_value"],":at":item["applies_to"],
                ":pi":item["product_ids"],":ci":item["category_ids"],
                ":moa":item["min_order_amount"],":mu":mu,
...
def get_active_offers(business_id):
    ...
    try:
        ...
        items = offers_table.scan(
            FilterExpression=Attr("business_id").eq(business_id) & Attr("is_active").eq(True)
        ).get("Items",[])
        valid = []
        for item in items:
            o = _map(item)
            if o["valid_from"]  and o["valid_from"]  > today: continue
            if o["valid_until"] and o["valid_until"] < today: continue
            if o["max_uses"] is not None and o["uses_count"] >= o["max_uses"]: continue
            valid.append(o)
